...
module.exports = (passport, db) => { 
	passport.use('local-login',new LocalStrategy((username, password, cb) => {
		db.query('SELECT id, username, fullname, password, type FROM users WHERE username=$1', [username], (err, result) => {
			if(err) {
			...
			if(result.rows.length > 0) {
				...
				bcrypt.compare(password, first.password, function(err, res) {
					if(res) {
						cb(null, { id: first.id, username: first.username, fullname: first.fullname, type: first.type })
					} else {				
						cb(null, false)
					}
				})
